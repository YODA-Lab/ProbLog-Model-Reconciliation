...
RESULT_LOG="${SCRIPT_DIR}/generic_results.log"
...
for index in ${!FACTS[@]}; do
    ...
    for complexity in "${COMPLEXITIES[@]}"; do
        ...
        echo "Testing: ${fact} facts, ${rule} rules, ${complexity} complexity, Condition B" | tee -a "$RESULT_LOG"
...
def run_model():
    ...
    result = subprocess.run(
        ['python3', 'main.py', '--query', 'd', '--KBa', '$KBa_A', '--KBh', '$KBh_A', '--tree_search', 'A', '--algorithm', 'all', '--cost_order', '[0.9801,0.8688,1.0202,1.1511]'],
...
def run_model():
    start = time.time()
    result = subprocess.run(
        ['python3', 'main.py', '--query', 'd', '--KBa', '$KBa_B', '--KBh', '$KBh_B', '--tree_search', 'A', '--algorithm', 'all', '--cost_order', '[0.9801,0.8688,1.0202,1.1511]'],
        capture_output=True, text=True
    )
    end = time.time()
    print(int((end - start) * 1000))
